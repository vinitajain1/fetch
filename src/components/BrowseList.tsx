...
const BrowseList: React.FC = () => {
    ...
    const count = Math.ceil(useSelector<RootStoreState,number>((store:RootStoreState)=>store.dogsSlice.total)/25);
    ...
    <div className="w-full p-8 flex flex-wrap gap-2 justify-center items-center">
        {status === LoadingStatus.LOADED ? dogs.length>0 ? (
            dogs.map((dog: Dog,index) => <BrowseCard index={index} key={dog.id} dog={dog} />)
